...
class HistoryStore:
    ...
    def update(self, episodes: list[Episode]) -> int:
        existing_ids = {(ep.get("source", "crunchyroll"), ep["episode_id"]) for ep in self._data["episodes"]}
        new_eps = [ep.to_dict() for ep in episodes if (ep.source, ep.episode_id) not in existing_ids]
    ...
    def series_summaries(self, since: Optional[str] = None) -> list[SeriesSummary]:
        ...
        for ep in self.all_episodes():
            if since and ep.watched_at and ep.watched_at < since:
                continue
            # Namespace non-Crunchyroll IDs so source-local numeric IDs cannot collide.
            prefix = "" if ep.source == "crunchyroll" else f"{ep.source}::"
            key = f"{prefix}{ep.series_id}::{ep.season_number}"
