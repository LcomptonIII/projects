...
class AniListExporter(BaseExporter):
    ...
    def validate_connection(self) -> dict:
        """Validate the saved token and return the authenticated AniList user."""
        data = self._gql(_VIEWER_QUERY, {})
        viewer = data.get("Viewer") or {}
        if not viewer.get("id") or not viewer.get("name"):
            raise ValueError("AniList did not return an authenticated Viewer.")
        return {"id": int(viewer["id"]), "name": str(viewer["name"])}
