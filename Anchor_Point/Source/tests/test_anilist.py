...
def test_validate_connection_returns_authenticated_viewer(monkeypatch):
    from src.exporters.anilist import AniListExporter
    exp = AniListExporter('local-test-token')
    monkeypatch.setattr(exp, '_gql', lambda query, variables: {'Viewer': {'id': 123, 'name': 'tester'}})
    assert exp.validate_connection() == {'id': 123, 'name': 'tester'}
