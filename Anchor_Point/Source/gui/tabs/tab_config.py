...
class ConfigTab:
    ...
    def _build(self, f: ctk.CTkFrame) -> None:
        ...
        ctk.CTkLabel(
            scroll,
            text=("FIRST-TIME SETUP: Create/authorize your AniList application, enter the Client ID and access token, "
                  "then click Configure AniList below. This configuration step must be completed before your first sync. "
                  "You only need to repeat it if the token or AniList application changes."),
            font=ctk.CTkFont(size=11, weight="bold"), text_color=("#8a5a00", "#f0b44c"),
            anchor="w", justify="left", wraplength=720,
        ).grid(row=row[0], column=0, columnspan=3, sticky="ew", padx=8, pady=(0, 8))
        row[0] += 1

        links = ctk.CTkFrame(scroll, fg_color="transparent")
        links.grid(row=row[0], column=0, columnspan=3, sticky="w", padx=8, pady=(0, 8))
        ctk.CTkButton(links, text="Open AniList Developer Page ↗", width=190,
                      command=lambda: webbrowser.open("https://anilist.co/settings/developer")).pack(side="left")
        ctk.CTkButton(links, text="Copy Redirect URL", width=150,
                      command=self._copy_anilist_redirect).pack(side="left", padx=(8, 0))
        self._al_copy_status = ctk.CTkLabel(links, text="", text_color=("gray45", "gray65"))
        self._al_copy_status.pack(side="left", padx=(8, 0))
        row[0] += 1
        ...
        self._al_status = ctk.CTkLabel(scroll, text="○ AniList not validated", anchor="w")
        self._al_status.grid(row=row[0], column=0, columnspan=3, sticky="w", padx=8, pady=(2, 5))
        row[0] += 1
        self._al_config_btn = ctk.CTkButton(scroll, text="Configure AniList", height=34, command=self._configure_anilist)
        self._al_config_btn.grid(row=row[0], column=0, columnspan=3, sticky="w", padx=8, pady=(0, 10))
        row[0] += 1
    ...
    def _save(self) -> None:
        ...
        new_al_token = val("al.access_token")
        old_al_token = cfg["exporters"]["anilist"].get("access_token", "")
        cfg["exporters"]["anilist"]["client_id"]    = val("al.client_id")
        cfg["exporters"]["anilist"]["access_token"] = new_al_token
        if new_al_token != old_al_token:
            cfg["exporters"]["anilist"].pop("validated_user_id", None)
            cfg["exporters"]["anilist"].pop("validated_username", None)
            cfg["exporters"]["anilist"].pop("validated_token_fingerprint", None)
        ...
        cfg.setdefault("ui", {})
        # Anchor Point 1.0 ships with the completed English UI only.
        cfg["ui"]["language"] = "en"
    ...
    def _copy_anilist_redirect(self) -> None:
        redirect = "https://anilist.co/api/v2/oauth/pin"
        self.frame.clipboard_clear()
        self.frame.clipboard_append(redirect)
        self.frame.update()
        self._al_copy_status.configure(text="Copied!")
        self.frame.after(1800, lambda: self._al_copy_status.configure(text=""))

    def _refresh_anilist_status(self) -> None:
        al = self.app.cfg.get("exporters", {}).get("anilist", {})
        token = al.get("access_token", "")
        valid = bool(token and al.get("validated_token_fingerprint") == self._token_fingerprint(token)
                     and al.get("validated_user_id") and al.get("validated_username"))
        if valid:
            self._al_status.configure(text=f"● AniList connected as {al.get('validated_username')}", text_color="#4caf50")
        elif token:
            self._al_status.configure(text="○ AniList token saved, but Configure AniList is still required", text_color="#d99a2b")
        else:
            self._al_status.configure(text="○ AniList not configured", text_color="#777777")

    def _configure_anilist(self) -> None:
        from tkinter import messagebox
        client_id = self._entries["al.client_id"].get().strip()
        token = self._entries["al.access_token"].get().strip()
        if not client_id or not token:
            messagebox.showerror("AniList", "Enter both the AniList Client ID and access token, then click Configure AniList.")
            return
        self._al_config_btn.configure(state="disabled", text="Validating…")
        self._al_status.configure(text="Validating AniList connection…", text_color="#d99a2b")

        def worker():
            try:
                from src.exporters.anilist import AniListExporter
                viewer = AniListExporter(token).validate_connection()
                def success():
                    al = self.app.cfg.setdefault("exporters", {}).setdefault("anilist", {})
                    al["client_id"] = client_id
                    al["access_token"] = token
                    al["validated_user_id"] = viewer["id"]
                    al["validated_username"] = viewer["name"]
                    al["validated_token_fingerprint"] = self._token_fingerprint(token)
                    self.app.save_config()
                    self._al_config_btn.configure(state="normal", text="Configure AniList")
                    self._refresh_anilist_status()
                    messagebox.showinfo("AniList", f"AniList configured successfully. Connected as {viewer['name']}.")
                self.frame.after(0, success)
            except Exception as ex:
                def failure(error=str(ex)):
                    al = self.app.cfg.setdefault("exporters", {}).setdefault("anilist", {})
                    al.pop("validated_user_id", None); al.pop("validated_username", None); al.pop("validated_token_fingerprint", None)
                    self.app.save_config()
                    self._al_config_btn.configure(state="normal", text="Configure AniList")
                    self._refresh_anilist_status()
                    messagebox.showerror("AniList", f"AniList validation failed:\n\n{error}")
                self.frame.after(0, failure)
