...
# CLI and GUI deliberately use the same implementation so scheduled-task fixes
# cannot drift between interfaces.

def _schedule_create(cmd: str, run_at: str) -> tuple[bool, str]:
    from src.scheduler import create_schedule
    return create_schedule(_TASK_NAME, cmd, run_at)

def _schedule_remove() -> tuple[bool, str]:
    from src.scheduler import remove_schedule
    return remove_schedule(_TASK_NAME)

def _query_task() -> tuple[bool, str]:
    from src.scheduler import query_schedule
    return query_schedule(_TASK_NAME)
